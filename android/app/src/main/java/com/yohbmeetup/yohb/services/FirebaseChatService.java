...
import com.yohbmeetup.yohb.activities.ChatActivity;
...
import com.yohbmeetup.yohb.receivers.ActionReceiver;
import com.yohbmeetup.yohb.utils.AESEncryption;
import com.yohbmeetup.yohb.utils.FirebaseUploader;
...
public class FirebaseChatService extends Service {
    ...
    public void onCreate() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(CHANNEL_ID_MAIN, "Sathichat service", NotificationManager.IMPORTANCE_LOW);
    ...
    private void notifyNewGroup(Group group) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(CHANNEL_ID_GROUP, "Sathichat new group notification", NotificationManager.IMPORTANCE_DEFAULT);
        ...
        public void onChildRemoved(DataSnapshot dataSnapshot) {
            ...
            if (chat != null) {
                ...
                if (realmList.size() == 0)
                    ...
                    chat.setLastRecipient(realmList.get(realmList.size() - 1).getRecipientId());
    ...
    private void saveMessage(Message message) {
        ...
        if (!message.isDelivered() && !message.getSenderId().equals(myId) && !helper.isUserMute(message.getSenderId()) && (Helper.CURRENT_CHAT_ID == null || !Helper.CURRENT_CHAT_ID.equals(userOrGroupId))) {
            ...
            if(message.getAttachmentType() == AttachmentTypes.VIDEO_CALL && (message.getDate() + 15000) > System.currentTimeMillis()){
                Intent intent = new Intent(getApplicationContext(), IncomingVideoCallActivity.class);
                intent.putExtra(CALL_ID, message.getSenderId());
                intent.putExtra("time", message.getDate());
                intent.putExtra("recipient_id", message.getRecipientId());
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                getApplicationContext().startActivity(intent);
            }
            else if(message.getAttachmentType() == AttachmentTypes.AUDIO_CALL && (message.getDate() + 15000) > System.currentTimeMillis()){
                Intent intent = new Intent(getApplicationContext(), IncomingAudioCallActivity.class);
                intent.putExtra(CALL_ID, message.getSenderId());
                intent.putExtra("time", message.getDate());
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                getApplicationContext().startActivity(intent);
                ...
                Uri defaultSoundUri = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION);
                String decryptedMessage = message.getBody();
                if(message.getAttachmentType() == AttachmentTypes.NONE_TEXT){
                    try {
                        ...
                        Log.d("Encrypted", "Decryption failed");
                    }
                }
                ...
                PendingIntent pIntentAction = PendingIntent.getBroadcast(this,1, intentAction, PendingIntent.FLAG_UPDATE_CURRENT);
